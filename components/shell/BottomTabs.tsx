...
export function BottomTabs({ tripId }: { tripId: string | null }) {
  ...
  const tabs = tripId ? TABS : [TABS[0], PROFILE_TAB];
          ...
          return (
            <li key={labelKey} className="relative">
              <Link
                href={href(tripId)}
                className={cn(
                  "relative flex flex-col items-center gap-0.5 rounded-2xl px-1.5 py-1.5 text-[10px] font-medium transition-colors",
                  active
