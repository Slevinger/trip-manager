...
export function TripDestinationsRoster({
  ...
  const cityGroups = useMemo(() => {
    ...
    const groups: Array<{ label: string; dests: Destination[] }> = grouped.stayGroups
      .map((g) => {
        const ids = new Set([
          ...
          ...g.memberPicks.map((p) => p.destinationId),
        ]);
        const dests = [...ids]
    ...
    // Destinations not covered by any stay group.
    const coveredIds = new Set(grouped.stayGroups.flatMap((g) =>
      [g.centerPick.destinationId, ...g.memberPicks.map((p) => p.destinationId)].filter(Boolean)
    ));
    const other = grouped.otherPicks
  ...
  return (
    <section className="rounded-xl border border-zinc-200 bg-white px-4 py-3 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <p className="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">
        {t("view.destinationsCount", { count: destinations.length })}
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {editable && onDeleteDestination ? (
                          <button
                            type="button"
                            disabled={!canDelete}
                            title={canDelete ? undefined : t("manage.deleteDestinationDisabledHint")}
                            onClick={() => {
                              if (!canDelete) return;
